...
<div class="card">
  <form action="<?= e(url('/admin/events/' . $event['id'])) ?>" method="POST" novalidate>
    <?= csrf_field() ?>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
      ...
      <div class="form-group">
        <label for="event-slug" class="form-label">Custom Slug</label>
        <input 
          type="text" 
          id="event-slug" 
          name="slug" 
          ...
          placeholder="e.g. listening-circle-1" 
          maxlength="191"
        >
        <?php if (!empty($errors['slug'])): ?>
          <span class="form-error"><?= e($errors['slug']) ?></span>
        <?php else: ?>
          <span class="form-hint">Unique URL slug within this campaign.</span>
        <?php endif; ?>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-4">
      ...
      <div class="form-group">
        <label for="event-status" class="form-label form-label-required">Status</label>
        <select 
          id="event-status" 
          name="status" 
    ...
    <div style="display: flex; align-items: center; justify-content: flex-end; gap: 0.75rem; border-top: 1px solid var(--border-color); padding-top: 1.25rem;">
      <a href="<?= e(url('/admin/events/' . $event['id'])) ?>" class="btn btn-outline">
        Cancel
      </a>
